...
This document describes the integration of DataForSEO's Content Generation API into the blog writer application.
...
2. **Next.js API Route** (`/api/blog-writer/generate`) receives the request and checks the flag
3. **DataForSEO Service** (`dataForSEOContentGeneration.generateBlogContent()`) is called
4. **DataForSEO API** is called with authenticated requests
5. **Response** flows back through the chain to the frontend

See [DATAFORSEO_USAGE_FLOW.md](./DATAFORSEO_USAGE_FLOW.md) for detailed flow documentation.
