...
# WITHOUT DataForSEO (using regular generation)
...
echo "=========================================="
echo "Using REGULAR generation (not DataForSEO)"
echo ""
echo "Endpoint: ${ENDPOINT}"
echo ""

# Test payload with 100 word target - WITHOUT DataForSEO
PAYLOAD=$(cat <<EOF
{
...
if [ "$HTTP_STATUS" -eq 200 ]; then
  ...
  CONTENT_WORD_COUNT=$(echo "$CONTENT" | wc -w | xargs)
  
  echo "Title: $TITLE"
  echo "Content Word Count: $CONTENT_WORD_COUNT"
  echo "Word Count (from metadata): $WORD_COUNT"
